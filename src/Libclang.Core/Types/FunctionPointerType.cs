...
namespace Libclang.Core.Types
{
    public class FunctionPointerType : TypeDefinition, IFunction
    {
        ...
        {
            var returnTypeEncoding = this.ReturnType.ToTypeEncoding(jsNameCalculator);
            var parameterTypeEncodings = this.Parameters.Select(p => p.Type.ToTypeEncoding(jsNameCalculator));
            if (this.IsBlock)
            {
