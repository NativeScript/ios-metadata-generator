...
namespace Libclang.Core.Parser
{
    ...
    public abstract class DeclarationVisitor : IDeclarationVisitor
    {
        ...
        public void VisitDeclaration(ClangIndex.DeclarationInfo declaration)
        {
            ...
            if (this.definitionParserMapper.TryGetValue(declaration.EntityInfo.Kind, out parser))
            {
                ...
                if (parsedDeclaration != null)
                {
                    ...
                    parsedDeclaration.Document = context.resolver.GetDocumentForDeclaration(parsedDeclaration);
        ...
        {
            System.Diagnostics.Debug.Assert(declaration.Location != null,
                "The location is not set on this declaration. " +
                "Declaration name: " + declaration.Name);

            DocumentDeclaration document = this.context.resolver.GetDocumentForDeclaration(declaration);
