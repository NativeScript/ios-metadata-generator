namespace TypeScript.Factory
{
    ...
    public class SourceTrackingDocumentationProvider : DocumentationProvider
    {
        ...
        public override string GetDoc(TS.MethodSignature method)
        {
            ...
            var methodMeta = method.Annotations.OfType<MT.MethodMeta>().FirstOrDefault();
            if (methodMeta != null)
            {
        ...
        public override string GetDoc(TS.PropertySignature property)
        {
            ...
            var propertyMeta = property.Annotations.OfType<MT.PropertyMeta>().FirstOrDefault();
        ...
        {
            var interfaceMeta = @class.Annotations.OfType<MT.InterfaceMeta>().FirstOrDefault();
        ...
        {
            var protocolMeta = @interface.Annotations.OfType<MT.ProtocolMeta>().FirstOrDefault();
            if (protocolMeta != null)
            {
                doc = Append(doc, string.Format("@protocol {0}", protocolMeta.Name));
            }

            var structMeta = @interface.Annotations.OfType<MT.StructMeta>().FirstOrDefault();
            if (structMeta != null)
            {
        ...
        {
            var category = typeScriptObject.Annotations.OfType<MT.CategoryMeta>().FirstOrDefault();
            if (category != null)
            {
        ...
        {
            var meta = typeScriptObject.Annotations.OfType<MT.Meta>().FirstOrDefault();
            if (meta != null)
            {
                if (Libclang.Core.Common.Version.IsSet(meta.IntroducedIn))
                {
                    doc = Append(doc, string.Format("@introduced {0}", meta.IntroducedIn));
                }

                if (Libclang.Core.Common.Version.IsSet(meta.ObsoletedIn))
                {
                    doc = Append(doc, string.Format("@obsolete {0}", meta.ObsoletedIn));
                }

                if (Libclang.Core.Common.Version.IsSet(meta.DeprecatedIn))
                {
                    doc = Append(doc, string.Format("@deprecated {0}", meta.DeprecatedIn));
                }
            }
        ...
        {
            var interfaceMeta = typeScriptObject.Annotations.OfType<MT.Meta>().FirstOrDefault();
            if (interfaceMeta != null && !string.IsNullOrWhiteSpace(interfaceMeta.Framework))
            {
                doc = Append(doc, string.Format("@framework {0}", interfaceMeta.Framework));
            }
